...
try:
    ...
    AntigravityRateLimitError = None  # type: ignore
    print(f"Warning: Antigravity modules not available: {e}")
...
class Config:
    def __init__(self):
        ...
        self.force_disable_streaming = (
            os.environ.get("FORCE_DISABLE_STREAMING", "false").lower() == "true"
        )
        self.emergency_disable_streaming = (
            os.environ.get("EMERGENCY_DISABLE_STREAMING", "false").lower() == "true"
        )

        # Antigravity settings
        self.antigravity_haiku_model = os.environ.get(
            "ANTIGRAVITY_HAIKU_MODEL", "antigravity-gemini-3-flash"
        )
        self.antigravity_sonnet_model = os.environ.get(
            "ANTIGRAVITY_SONNET_MODEL", "antigravity-claude-sonnet-4-5-thinking"
        )
        self.antigravity_opus_model = os.environ.get(
...
class ModelManager:
    ...
    def get_antigravity_model(self, original_model: str) -> Optional[str]:
        """
        Get the Antigravity model name for a Claude model.
        ...
        if "haiku" in model_lower:
            ...
            return self.config.antigravity_sonnet_model
...
# Antigravity initialization (if enabled and available)
auth_manager = None
quota_manager = None
...
if config.use_antigravity and ANTIGRAVITY_AVAILABLE:
    try:
        ...
        quota_manager = QuotaManager(
            ...
            use_antigravity=config.use_antigravity,
        )
        print(f"✅ Antigravity enabled: {auth_manager.account_count()} account(s) loaded")
    except Exception as e:
        print(f"⚠️ Failed to initialize Antigravity: {e}")
        config.use_antigravity = False
elif config.use_antigravity and not ANTIGRAVITY_AVAILABLE:
    print("⚠️ Antigravity requested but dependencies not available")
    config.use_antigravity = False
...
def clean_gemini_schema(schema: Any) -> Any:
    ...
    if isinstance(schema, dict):
        ...
        # Normalize type: some schemas use ["string", "null"], which Antigravity rejects
        if isinstance(schema.get("type"), list):
            types = [t for t in schema["type"] if isinstance(t, str) and t != "null"]
        ...
        # Heuristic: collapse trivial anyOf/oneOf patterns like [{...}, {"type":"null"}]
        for union_key in ("anyOf", "oneOf"):
            if isinstance(schema.get(union_key), list) and schema[union_key]:
                non_null = [
                    s for s in schema[union_key] if isinstance(s, dict) and s.get("type") != "null"
                ]
                if len(non_null) == 1:
...
def convert_litellm_to_anthropic(
    ...
    try:
        ...
        return MessagesResponse(
            ...
            role=Constants.ROLE_ASSISTANT,
            content=[
                ContentBlockText(type=Constants.CONTENT_TEXT, text="Response conversion error")
            ],
...
async def handle_streaming_with_recovery(
    response_generator, original_request: MessagesRequest, input_tokens: int
):
    ...
    try:
        ...
        while True:
            try:
                ...
                if isinstance(chunk, str):
                    ...
                    if is_malformed_chunk(chunk):
                        ...
                        if malformed_chunks_count > max_malformed_chunks:
                            logger.error(
                                f"Too many malformed chunks ({malformed_chunks_count}), terminating stream"
                            )
                            stream_terminated_early = True
                            break
                        continue
                ...
                if delta_tool_calls:
                    for tc_chunk in delta_tool_calls:
                        ...
                        if tc_chunk.function.arguments:
                            current_tool_calls[tool_call_id]["args_buffer"] += (
                ...
                logger.debug(
                    f"JSON parsing error (attempt {consecutive_errors}/{max_consecutive_errors}): {parse_error}"
                )

                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        f"Too many consecutive parsing errors ({consecutive_errors}), terminating stream"
                    )
                    stream_terminated_early = True
                    break
                continue
                ...
                if (
                    "Error parsing chunk" in error_msg
                    ...
                    if consecutive_errors >= max_consecutive_errors:
                        logger.error(
                            f"Too many consecutive API errors ({consecutive_errors}), terminating stream"
                        )
                        stream_terminated_early = True
                ...
                consecutive_errors += 1
                logger.error(
                    f"Unexpected streaming error (attempt {consecutive_errors}/{max_consecutive_errors}): {general_error}"
                )

                if consecutive_errors >= max_consecutive_errors:
                    logger.error(
                        f"Too many consecutive errors ({consecutive_errors}), terminating stream"
                    )
                    stream_terminated_early = True
                    break
...
async def _handle_antigravity_request(
    antigravity_model: str,
    request: MessagesRequest,
    raw_request: Request,
):
    """
    Handle a request through Antigravity quota manager.

    Args:
        antigravity_model: The Antigravity model name to use
        request: The Anthropic-format request
        raw_request: The FastAPI Request object

    Returns:
        StreamingResponse or dict response
    """
    import uuid

    # Convert Anthropic request to LiteLLM format for message conversion
    litellm_request = convert_anthropic_to_litellm(request)

    # Prepare parameters for Antigravity client
    params = {
    ...
    log_request_beautifully(
        ...
        f"antigravity:{antigravity_model}",
...
# Main endpoint (Antigravity-only)
@app.post("/v1/messages")
async def create_message(request: MessagesRequest, raw_request: Request):
    try:
        body = await raw_request.json()
        raw_model = body.get("model", request.model)

        logger.debug(
            f"📊 Processing request: Raw={raw_model}, Original={request.original_model}, Effective={request.model}, Stream={request.stream}"
        )
        ...
        antigravity_model = model_manager.get_antigravity_model(original_model)

        logger.debug(
            f"Decision: antigravity_model={antigravity_model}, quota_manager={quota_manager is not None}"
        )

        if antigravity_model and quota_manager:
            logger.info(f"USING ANTI-GRAVITY: {original_model} -> {antigravity_model}")
            return await _handle_antigravity_request(antigravity_model, request, raw_request)

        raise HTTPException(
            status_code=400,
            detail=(
                "Antigravity-only proxy: requested model was not routed to Antigravity. "
                "Use a Claude model name containing haiku/sonnet/opus (or update model mappings)."
            ),
        )
...
async def health_check():
    try:
        ...
        # Add Antigravity status if enabled
        if config.use_antigravity and quota_manager:
            health_status["antigravity"] = quota_manager.get_status()
...
async def antigravity_status():
    """Get detailed Antigravity quota status."""
    ...
    if not quota_manager:
        return {
            "enabled": False,
            "message": "Antigravity configured but quota manager not initialized.",
        }
...
async def root():
    config_data = {
        ...
        "config": {
            ...
            "antigravity": {
                "enabled": config.use_antigravity,
                "haiku_model": config.antigravity_haiku_model if config.use_antigravity else None,
                "sonnet_model": config.antigravity_sonnet_model if config.use_antigravity else None,
                "opus_model": config.antigravity_opus_model if config.use_antigravity else None,
                "accounts_loaded": auth_manager.account_count() if auth_manager else 0,
            },
        },
        "endpoints": {
    ...
    # Add quota status if Antigravity is enabled
    if config.use_antigravity and quota_manager:
        config_data["config"]["antigravity"]["quota"] = quota_manager.get_status()
...
def main():
    # Handle --antigravity-login command
    if len(sys.argv) > 1 and sys.argv[1] == "--antigravity-login":
        if not ANTIGRAVITY_AVAILABLE:
            print("❌ Antigravity modules not available. Install missing dependencies:")
            print("   pip install aiohttp google-auth-oauth-lib google-auth")
            sys.exit(1)

        print("🔐 Starting Antigravity OAuth authentication...")
        asyncio.run(authenticate_and_store(auth_manager))
        sys.exit(0)
    ...
    if config.use_antigravity:
        if auth_manager:
            print(f"   Antigravity: Enabled ({auth_manager.account_count()} account(s))")
        else:
            print(f"   Antigravity: Enabled but not initialized")
    else:
        print(f"   Antigravity: Disabled")
    print("")
