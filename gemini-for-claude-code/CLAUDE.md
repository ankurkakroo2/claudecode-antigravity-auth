...
This is **gemini-for-claude-proxy**, a proxy server that translates between Anthropic's Messages API format (used by Claude Code) and Google's Gemini API. It enables using Claude Code CLI with Google Gemini models as the backend.

**Key capability:** The proxy includes Antigravity OAuth integration for higher-quota Google AI models.
...
- `quota_manager.py` - Quota management with automatic fallback

**Legacy Standalone:**
- `server.py` - Single-file FastAPI application (can run directly)
...
- Schema cleaning (removes unsupported fields like `additionalProperties`, `default`)

**Gemini → Anthropic:**
- Streaming response with error recovery
- Tool calls → Anthropic `tool_use` blocks
- Stop reason mapping (STOP → end_turn, MAX_TOKENS → max_tokens)
...
| `*sonnet*` | `gemini-1.5-pro-latest` | `antigravity-claude-sonnet-4-5-thinking` |
...
- `BIG_MODEL` / `SMALL_MODEL` - Model mappings
- `HOST` / `PORT` - Server binding
- `LOG_LEVEL` - Logging verbosity
- `USE_ANTIGRAVITY` - Enable OAuth integration

**Config File (~/.gclaude/config.json):**
- Created by `gclaude init` command
- Stores model mappings and OAuth settings
...
The proxy has sophisticated error recovery:
- Malformed JSON chunks are buffered and retried
- Exponential backoff (configurable via `MAX_STREAMING_RETRIES`)
- Automatic fallback to non-streaming mode
- Connection error recovery for Gemini 500 errors

If experiencing streaming issues, set `FORCE_DISABLE_STREAMING=true` in `.env`
...
3. Tokens stored in `~/.config/gclaude/antigravity-accounts.json`
4. Access token automatically refreshed when expired
5. Fallback to Gemini API key if OAuth fails
...
- `GET /antigravity-status` - OAuth and quota status
...
**OAuth fails:** Remove `~/.config/gclaude/antigravity-accounts.json` and re-auth
**Streaming errors:** Check logs with `python -m gclaude logs`, try disabling streaming
**API key issues:** Verify format (starts with "AIza", 39 chars), test with `/test-connection`
...
- **Never commit OAuth tokens** - `~/.config/gclaude/antigravity-accounts.json`
- **Test health** before using: `curl http://localhost:8082/health`
- **Check logs** when troubleshooting: `python -m gclaude logs`
- **Use gclaude CLI** for easier management vs direct `server.py`

---

## Claude Code: Best Practices for Effective Collaboration

This section outlines best practices for working with Claude Code to ensure efficient and successful software development tasks.
